...
jit.findPlugin = function (taskName) {
  ...
  if (this.mappings.hasOwnProperty(taskName)) {
    var task = this.findStatic(taskName);
    if (task) {
      return task;
    }
  }
  ...
  if (jit.pluginsRoot) {
    for (var p = PREFIXES.length; p--;) {
      ...
      var findPath = path.resolve(path.join(jit.pluginsRoot, pluginName, 'tasks'));
      if (fs.existsSync(findPath)) {
        return jit.loadPlugin(pluginName, findPath);
      }
    }
  }
  ...
  taskPath = jit.findUp(path.resolve(), function (cwd) {
    for (var p = PREFIXES.length; p--;) {
      pluginName = PREFIXES[p] + dashedName;
      var findPath = path.join(cwd, 'node_modules', pluginName, 'tasks');
      if (fs.existsSync(findPath)) {
        return findPath;
      }
    }
...
//TODO: Refactoring
jit.findStatic = function (taskName) {
  var pluginName = this.mappings[taskName];
  var taskPath;
  if (pluginName.indexOf('/') >= 0 && pluginName.indexOf('@') !== 0) {
    ...
    if (jit.pluginsRoot) {
      taskPath = path.resolve(path.join(jit.pluginsRoot, pluginName, 'tasks'));
      if (fs.existsSync(taskPath)) {
        return jit.loadPlugin(pluginName, taskPath);
      }
    }
