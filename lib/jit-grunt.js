...
jit.findPluginPath = function(pluginName) {
  ...
  if (jit.pluginsRoot.match(/node_modules/)) {
    taskPath = findup(jit.pluginsRoot, path.join('node_modules', pluginName, 'tasks'));
    if (taskPath) {
      return taskPath;
    }
  }
...
function findup(cwd, pattern) {
  var taskPath = path.join(cwd, pattern);
  if (fs.existsSync(taskPath)) {
    return taskPath;
  }
  var next = path.resolve(cwd, '..');
  return next !== cwd ? findup(next, pattern) : null;
}
